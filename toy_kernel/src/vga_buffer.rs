...
lazy_static! {
    pub static ref WRITER: Mutex<Writer> = Mutex::new(Writer::new(Color::Yellow, Color::Black));
}
...
impl Writer {
    pub fn new(text_color: Color, background_color: Color) -> Self {
        Self {
            column_position: 0,
            color_code: ColorCode::new(text_color, background_color),
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                // 0x20..=0x7e are the printable ascii chars (and newline)
